...
import {
    ...
    ChevronRight, Menu, X, Bell, CheckCheck,
} from 'lucide-react'
...
export default function CustomerLayout() {
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center px-4">
            ...
            <div className="relative z-10 w-full max-w-sm bg-[#0d2a4a] border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col items-center gap-4">
                ...
                <div className="text-center">
                    ...
                    <p className="text-blue-200 text-sm mt-1 leading-snug">Are you sure you want to log out of your session?</p>
                </div>
    ...
    // ── Notification Panel ─────────────────────────────────────────────────
    const NotifPanel = () => (
        <div className="absolute right-0 top-11 w-80 bg-white rounded-2xl border border-gray-200 shadow-2xl z-[39] overflow-hidden">
    ...
    // ── Topbar (shared) ────────────────────────────────────────────────────
    const Topbar = () => (
        <header className="min-h-[52px] bg-white border-b border-gray-200 flex items-center justify-between px-4 shrink-0 shadow-sm z-10">
            ...
            <div className="flex flex-col h-[100dvh] bg-[#f0f4f8] overflow-hidden">
                ...
                <nav className={`bg-[#0d2a4a] border-t border-white/10 flex items-center justify-around shrink-0
                    ...
                    <button onClick={() => setShowLogoutModal(true)}
                        className="flex flex-col items-center gap-0.5 px-2 py-1 rounded-lg transition-all min-w-[44px] text-red-300 hover:text-red-200">
    ...
    return (
        <div className="flex h-[100dvh] bg-[#f0f4f8] overflow-hidden">
            <aside className={`flex flex-col justify-between bg-[#0d2a4a] text-white shrink-0 transition-all duration-300 ease-in-out overflow-hidden
                ...
                <div className={`px-2 pb-4 flex flex-col gap-1 ${sidebarIconOnly ? 'items-center' : ''}`}>
                    <button onClick={() => setShowLogoutModal(true)} title={sidebarIconOnly ? 'Logout' : undefined}
                        className={`flex items-center gap-3 rounded-xl text-sm font-medium w-full text-red-300 hover:bg-red-500/10 hover:text-red-200 transition-all duration-150
