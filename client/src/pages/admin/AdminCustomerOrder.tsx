...
import {
    ...
    History, ShoppingBag, ChevronRight,
} from 'lucide-react'
...
interface Order {
    ...
    order_status: 'confirmed' | 'preparing' | 'delivering' | 'cancelled' | 'returned'
    payment_type: string
...
const STATUS_CFG: Record<string, { label: string; color: string; bg: string; border: string; dot: string; icon: any }> = {
    ...
    delivering: { label: 'Delivering', color: 'text-purple-600', bg: 'bg-purple-50', border: 'border-purple-200', dot: 'bg-purple-400', icon: Truck },
...
const GCashModal = ({ order, onClose, onVerify, API }: {
    ...
    const handle = async (status: 'verified' | 'rejected') => {
        setBusy(true)
        await onVerify(order.order_id, status)
        setBusy(false)
        onClose()
    }
                ...
                <div className="p-5 flex flex-col gap-4">
                    ...
                    {order.proof_image_path ? (
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="rounded-xl overflow-hidden border border-gray-200 relative">
                                <img
                                    src={order.proof_image_path.startsWith('http') ? order.proof_image_path : `${API}${order.proof_image_path}`}
                                    alt="GCash Receipt"
                    ...
                    {order.payment_status !== 'pending' && (
                        <div className={`flex items-center gap-2 px-4 py-3 rounded-xl text-xs font-semibold border
                            ${PAY_CFG[order.payment_status]?.bg} ${PAY_CFG[order.payment_status]?.color} ${PAY_CFG[order.payment_status]?.border}`}>
                            <CheckCircle2 size={13} />
                            Payment already {order.payment_status}
                        </div>
                    )}
                </div>
            </div>
...
const ReturnModal = ({ order, onClose, onResolve }: {
    ...
    const handle = async (status: 'approved' | 'rejected') => {
        setBusy(true)
        await onResolve(order.order_id, status)
        setBusy(false)
        onClose()
    }
            ...
            <div className="relative z-10 w-full max-w-sm bg-white rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div className="p-5 flex flex-col gap-4">
                    ...
                    {isResolved ? (
                        ...
                        <div className="flex flex-col gap-2">
                            <p className="text-[10px] text-gray-400">Approving will confirm the return. Rejecting will restore the order to delivering status.</p>
                            <div className="flex gap-2">
                                <button onClick={() => handle('rejected')} disabled={busy}
                                    className="flex-1 py-2.5 rounded-xl bg-red-50 hover:bg-red-100 text-red-500 text-xs font-bold border border-red-100 transition-all flex items-center justify-center gap-1.5 disabled:opacity-60">
            ...
            <div className="relative z-10 w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden max-h-[92vh] flex flex-col">
                ...
                <div className="overflow-y-auto flex-1 p-5 flex flex-col gap-4">
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                        ...
                        {/* Payment badge — clickable if gcash */}
                        {isOnline ? (
                    ...
                    <div className="rounded-xl overflow-hidden border border-gray-100">
                        ...
                        {order.items?.map(item => (
                            <div key={item.order_item_id} className="flex items-center gap-3 px-4 py-3 border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
...
// ── Order Row ──────────────────────────────────────────────────────────────
const OrderRow = ({ order, onOpen, API }: { order: Order; onOpen: () => void; API: string }) => {
    const status = STATUS_CFG[order.order_status] ?? STATUS_CFG.confirmed
    const payment = PAY_CFG[order.payment_status] ?? PAY_CFG.pending
    ...
    return (
        <tr className="hover:bg-gray-50/60 transition-colors cursor-pointer" onClick={onOpen}>
            <td className="px-5 py-3.5">
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        <p className="font-bold text-gray-800 text-xs font-mono">{order.order_reference}</p>
                        <p className="text-[10px] text-gray-400">{order.item_count} item(s)</p>
                    </div>
                </div>
            </td>
            <td className="px-4 py-3.5">
                <p className="font-semibold text-gray-800 text-xs">{order.customer_name || 'Walk-in'}</p>
                <p className="text-[10px] text-gray-400 truncate max-w-[120px]">{order.customer_email || '—'}</p>
            </td>
            <td className="px-4 py-3.5 text-center">
                <p className="text-xs text-gray-500">{formatDate(order.created_at)}</p>
            </td>
            <td className="px-4 py-3.5 text-center">
                <span className="text-[10px] font-bold text-gray-500 uppercase bg-gray-100 px-2 py-0.5 rounded-lg">{order.payment_mode}</span>
            </td>
            <td className="px-4 py-3.5 text-center">
                <span className="font-black text-gray-800 text-xs">{fmt(order.total_amount)}</span>
            </td>
            <td className="px-4 py-3.5 text-center">
                {isGcashPending ? (
                    <span className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-600 bg-amber-50 border border-amber-200 px-2 py-1 rounded-lg">
                ...
                {hasReturn && (
                    <span className="block text-[9px] font-bold text-orange-500 mt-0.5 animate-pulse">⟳ Return Req.</span>
                )}
            </td>
            <td className="px-4 py-3.5 text-center">
                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold border ${status.bg} ${status.color} ${status.border}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${status.dot}`} />
                    {status.label}
                </span>
            </td>
            <td className="px-4 py-3.5 text-center">
                <div className="p-1.5 rounded-lg bg-[#0d2a4a]/8 hover:bg-[#0d2a4a]/15 text-[#0d2a4a] transition-all inline-flex">
                    <Eye size={14} />
                </div>
            </td>
        </tr>
...
export default function AdminCustomerOrder() {
    ...
    const statuses = ['all', 'confirmed', 'preparing', 'delivering', 'cancelled', 'returned']
        ...
        <div className="flex flex-col gap-4 pb-10">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex items-center gap-2">
                    {(pendingGcash > 0 || pendingReturns > 0) && (
                        <div className="flex gap-1.5">
                            {pendingGcash > 0 && (
                                <span className="flex items-center gap-1 text-[10px] font-bold text-amber-600 bg-amber-50 border border-amber-200 px-2.5 py-1.5 rounded-xl">
                                    <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />
                                    {pendingGcash} GCash pending
                                </span>
                            )}
                            {pendingReturns > 0 && (
                                <span className="flex items-center gap-1 text-[10px] font-bold text-orange-600 bg-orange-50 border border-orange-200 px-2.5 py-1.5 rounded-xl">
                                    <RotateCcw size={10} className="animate-spin" style={{ animationDuration: '3s' }} />
                                    {pendingReturns} return(s)
                                </span>
                            )}
                        </div>
                    )}
                    <span className="text-sm font-bold text-[#0d2a4a] bg-white border border-gray-100 px-3 py-1.5 rounded-xl shadow-sm">
            ...
            {view === 'history' && (
                <div className="flex items-center gap-2 px-4 py-3 bg-blue-50 border border-blue-100 rounded-xl text-xs text-blue-600">
                    ...
                    Showing orders from previous days. Active/delivering orders are in the Active tab.
                </div>
            )}
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-gray-100 bg-gray-50/60">
                                    {['Order', 'Customer', 'Date', 'Type', 'Total', 'Payment', 'Status', ''].map(h => (
                                        <th key={h} className="px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-wider text-left first:px-5">
                                            {h}
                                        </th>
                                    ))}
                                </tr>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {orders.map(order => (
                                    <OrderRow key={order.order_id} order={order} API={API}
